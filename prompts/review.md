...
- Creates or updates: `aidd/work/REVIEW.md`
- Runs validation: `bash scripts/review-check.sh`
...
3. **Document sensitive points requiring human decision**
   - Check if `aidd/work/SENSITIVE_NOTES.md` exists (created during implementation)
   - If exists:
...
4. **Document test evidence**
   ...
   - **Mandatory**: Include `bash scripts/aidd-check.sh` results
   - **If UI changes**: Include `bash scripts/aidd-verify-ui.sh` results
...
5. **Reference security checklists** (domain-specific)
   - Read `aidd/memory/techContext.md` to determine Domain
    - If Domain is "web3":
      - Reference `aidd/review/review-checklist-web3.md`
    ...
    - If Domain is "ml":
      - Reference `aidd/review/review-checklist-ml.md`
...
- After review approval, update `aidd/memory/activeContext.md` with completion details
- Do not proceed to PR/merge if Verdict is CHANGES_REQUESTED

## Definition of Done

- REVIEW.md exists at `aidd/work/REVIEW.md`
...
- Review validation passes: `bash scripts/review-check.sh` returns exit code 0
