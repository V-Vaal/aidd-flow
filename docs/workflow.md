...
- Command: `@aidd.audit` or `@aidd.start` (exploratory mode)
- Creates: `.cursor/work/AUDIT.md`
- Use findings to inform planning

**1. Intake**
- Command: `@aidd.intake`
- Creates: `.cursor/work/INTAKE.md`
- Define goal, scope, constraints, risks, acceptance criteria
- See [INTAKE specification](intake.md) for structure

**2. Plan**
- Command: `@aidd.plan`
- Creates: `.cursor/work/PLAN.md`
- Document steps, files to touch, tests, rollback strategy
- See [PLAN specification](technical-plan.md) for structure

**3. Validate Plan**
- Command: `@aidd.validate` or `bash .cursor/scripts/validate-plan.sh`
...
- `bash .cursor/scripts/aidd-verify-ui.sh` (if UI changes)

**6. Review**
- Command: `@aidd.review`
- Creates: `.cursor/work/REVIEW.md` with Verdict (APPROVE | CHANGES_REQUESTED)
- Validate: `bash .cursor/scripts/review-check.sh`
- **Gate**: Task not done until Verdict is APPROVE

**7. Update Active Context**
- Command: `@aidd.updateActiveContext`
- Updates: `.cursor/memory/activeContext.md`
...
**All queries are human-defined** via `.cursor/work/github-signals.config.yml`. This ensures:
...
**Environment setup:**
- Cursor inherits environment variables from the shell that launched it
- Dotenv files (`.env`) are NOT automatically loaded
- Export tokens before launching Cursor: `export GITHUB_TOKEN=your_token_here`
...
- Persisted in `.cursor/work/RUN_STATE.json` after each successful run
...
**Reset Gate behavior:**
When target changes:
1. Creates archive folder: `.cursor/work/_archive/{timestamp}-{repo}-{type}-{number}/`
...
When updating `.cursor/work/github-signals.config.yml`, always use full replacement patterns:
...
**Ensure you run scripts from project root**, not from `.cursor/scripts/`.

### MCP shows unavailable

**Possible causes:**
- MCP server not configured in Cursor
...
**Solutions:**
1. Export token before launching Cursor:
   ```bash
   export GITHUB_TOKEN=your_token_here
   # Relaunch Cursor from this shell
   ```
...
3. Or add to shell profile:
   ...
   export GITHUB_TOKEN=your_token_here
   # Then restart shell and relaunch Cursor
   ```
...
**Cursor Commands:**
- `@aidd.start` - Start workflow (targeted or exploratory mode)
- `@aidd.audit` - Repository audit
- `@aidd.intake` - Create INTAKE.md
- `@aidd.plan` - Create PLAN.md
- `@aidd.validate` - Validate PLAN.md
- `@aidd.review` - Create REVIEW.md
- `@aidd.updateActiveContext` - Update activeContext.md
- `@aidd.prMessage` - Generate PR message

**Scripts:**
- `bash .cursor/scripts/validate-plan.sh` - Validate PLAN.md structure
- `bash .cursor/scripts/aidd-check.sh` - Comprehensive validation
- `bash .cursor/scripts/aidd-check.sh --plan` - Include PLAN.md validation
- `bash .cursor/scripts/aidd-check.sh --review` - Include REVIEW.md validation
- `bash .cursor/scripts/review-check.sh` - Validate REVIEW.md
- `bash .cursor/scripts/aidd-verify-ui.sh` - UI verification (if frontend)
- `bash .cursor/scripts/validate-rules.sh` - Validate rules structure

**Artifacts:**
- `.cursor/work/AUDIT.md` - Repository analysis
- `.cursor/work/INTAKE.md` - Requirements and constraints
- `.cursor/work/PLAN.md` - Technical implementation plan
- `.cursor/work/REVIEW.md` - Review verdict and evidence
- `.cursor/work/github-signals.md` - GitHub signals (targeted mode)
- `.cursor/work/github-signals.config.yml` - GitHub signals configuration
