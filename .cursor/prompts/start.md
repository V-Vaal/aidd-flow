...
- Check if `.cursor/work/github-signals.config.yml` exists
...
**Check RUN_STATE:**
- Read `.cursor/work/RUN_STATE.json` if it exists
...
- **If RUN_STATE exists AND target identity differs:**
  ...
  - **Trigger Reset Gate (Option A: Automatic Archive + Reset):**
    1. Create archive folder: `.cursor/work/_archive/{timestamp}-{prev_repo}-{prev_type}-{prev_number}/`
    ...
    4. Log reset action:
       - Console: "Target changed from {prev_type} #{prev_number} to {new_type} #{new_number} -> archived previous artifacts to .cursor/work/_archive/{archive_folder}/ -> fresh run started"
...
**Write config:**
- Update or create `.cursor/work/github-signals.config.yml`
...
Create `.cursor/work/TARGET.md` with facts only:
```markdown
# Target
...
- Create or update `.cursor/work/RUN_STATE.json`:
  ```json
  {
    "repo": "{owner/name}",
    "type": "{issue|pr}",
...
1. **aidd.githubSignals**
   - Reads `.cursor/work/github-signals.config.yml`
   ...
   - Produces `.cursor/work/github-signals.md` with "Analysis Keywords" and "Related Context" sections (when applicable)
   - Uses GitHub MCP if available, with fallback to REST API, gh CLI, or manual instructions

2. **aidd.auditSeed**
   - Reads `.cursor/work/AUDIT.md` (create template if missing)
   - Reads `.cursor/work/github-signals.md` (from step 1)
   - Populates `.cursor/work/AUDIT.md` with facts and analysis

3. **aidd.intake**
   - Follows `.cursor/prompts/intake.md`
   - Produces `.cursor/work/INTAKE.md` (draft)

4. **aidd.plan**
   - Follows `.cursor/prompts/plan.md`
...
- [ ] Review `.cursor/work/AUDIT.md` for audit findings
- [ ] Review `.cursor/work/INTAKE.md` for requirements
- [ ] Review `.cursor/work/PLAN.md` for technical plan
- [ ] Go to UI ChatGPT for architect validation (AUDIT/INTAKE/PLAN)
- [ ] After validation, proceed with implementation
- [ ] Run `aidd.review` after implementation
```
...
- Check if `.cursor/work/AUDIT.md` exists
- If missing, create with standard template structure (from `aidd.auditSeed` reference)

**2B.3: Execute audit**

Run **aidd.auditSeed**:
- Reads `.cursor/work/AUDIT.md`
- Analyzes repository structure
- Populates factual sections with evidence
- **Do NOT require** `.cursor/work/github-signals.md` (it may not exist)

**2B.4: Mark AUDIT.md as Exploratory**

After `aidd.auditSeed` completes, update `.cursor/work/AUDIT.md`:
...
- [ ] Review `.cursor/work/AUDIT.md` for findings
...
- [ ] Convert finding to Targeted run:
  ...
  - Run `aidd.start` in Targeted mode with issue/PR number
...
- `.cursor/work/TARGET.md` (optional)
- `.cursor/work/github-signals.config.yml` (updated/created)
- `.cursor/work/github-signals.md` (from aidd.githubSignals)
- `.cursor/work/AUDIT.md` (populated by aidd.auditSeed)
- `.cursor/work/INTAKE.md` (draft, from aidd.intake)
- `.cursor/work/PLAN.md` (draft, from aidd.plan)

### Exploratory Mode Artifacts
- `.cursor/work/TARGET.md` (optional)
- `.cursor/work/AUDIT.md` (populated by aidd.auditSeed, marked as Exploratory)
- `.cursor/work/AUDIT.md` includes "Findings / Backlog candidates" section
