...
- `.cursor/memory/techContext.md` (technical stack and context)
- `.cursor/memory/systemPatterns.md` (architectural patterns, if available)
- Repository codebase for understanding current implementation

## Output Artefact

- Creates or updates: `.cursor/work/PLAN.md`
...
2. **Review system context**
   ...
   - Read `.cursor/memory/techContext.md` for technical stack
   - Read `.cursor/memory/systemPatterns.md` for design patterns
...
7. **Define tests and checks**
   ...
   - **Mandatory**: Include `bash .cursor/scripts/aidd-check.sh`
   - **If UI changes**: Include `bash .cursor/scripts/aidd-verify-ui.sh`
...
12. **Plan checklist** (for validation)
    ...
    - Run `bash .cursor/scripts/validate-plan.sh` to validate structure

## Implementation Guidance

**During implementation, maintain sensitive points tracking:**

- Create or update `.cursor/work/SENSITIVE_NOTES.md` (facts-only) when encountering:
...
- **Rules:**
  ...
  - This file will be referenced during review (see review.md step 3)
...
- PLAN.md exists at `.cursor/work/PLAN.md`
...
- PLAN.md validation passes: `bash .cursor/scripts/validate-plan.sh` returns exit code 0
