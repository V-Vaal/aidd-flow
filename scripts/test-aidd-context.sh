...
# Setup temp workspace structure matching template layout
mkdir -p "$TMPDIR/template/scripts"
mkdir -p "$TMPDIR/template/.cursor/memory"

# Copy script under test into temp workspace
cp "$CONTEXT_SCRIPT" "$TMPDIR/template/scripts/aidd-context.sh"
chmod +x "$TMPDIR/template/scripts/aidd-context.sh"

# Create a memory file with recognizable test marker
echo "TEST_MEMORY_BANK" > "$TMPDIR/template/.cursor/memory/projectbrief.md"
...
if bash "$TMPDIR/template/scripts/aidd-context.sh" > "$DEFAULT_OUT" 2>&1; then
...
if bash "$TMPDIR/template/scripts/aidd-context.sh" --verbose > "$VERBOSE_OUT" 2>&1; then
